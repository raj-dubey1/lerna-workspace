...
export class CometChatListItem extends LitElement {
  ...
  render() {
    ...
    return html` 
    <div class="cc__list__item" 
style=" ${styleMap(this.listItemStyles.listItemStyle())}" data-id=${this.id} @mouseenter=${this.hideShowTail} @mouseleave=${this.hideShowTail} @click=${this.setActiveItem}>
<div class="cc__item__thumbnail">
...
<div class="cc__item__details">
<div class="cc__item__title" style="${styleMap(this.listItemStyles.itemDetailStyle(this.style))}">
  <div class="cc__item__innerTitle" style="${styleMap(this.listItemStyles.titleStyle(this.style))}">
  ${this.title}
  </div>
  <div> <slot name="subtitleView"></slot> </div>
</div>
<div class="cc__tail__view">
${!this.isHovering ? html`<slot name="tailView"> </slot>` : ""}
</div>
<div class="cc__menu__view">
${this.isHovering ? html`<slot name="menuView"> </slot>` : ""}
</div>
</div>
</div>
${!this.hideSeparator ? html`<div class="cc__divider" style=${styleMap(this.listItemStyles.dividerStyle())}> <cometchat-divider .style=${this.dividerStyles}></cometchat-divider> </div>` : nothing}
    `;
  }
  listItemStyles = {
    listItemStyle: () => {
