...
export class CometChatSearch extends LitElement {
  ...
  @property({ type: Object }) searchCallback: Function = () => { };
  ...
  static styles = [
    ...
    .cc__search__icon{margin-top:3px;}
    `
  ];
  render() {
    return html`<div class="cc__search" style= ${styleMap(this.searchStyle())} >
          <cometchat-icon class="cc__search__icon"  .URL=${this.searchIconURL} .style=${this.iconStyle}></cometchat-icon>
          <cometchat-input class="cc__search__input" .searchCallback=${this.keyDownEvent}  placeholderText=${this.placeholderText} value=${this.searchText} .style=${this.inputStyle}></cometchat-input>
</div>`;
  }
  searchStyle() {
    return {
