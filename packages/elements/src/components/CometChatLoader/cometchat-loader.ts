...
export class CometChatLoader extends LitElement {
  ...
  render() {
    if (this.iconURL) {
      return html` <div class="cc__loader" style=${styleMap(this.inputStyle())}><cometchat-icon .URL=${this.iconURL} .name=${this.name} .style=${this.iconStyle}></cometchat-icon></div>`;
    }
  }
  inputStyle = () => {
    return {
