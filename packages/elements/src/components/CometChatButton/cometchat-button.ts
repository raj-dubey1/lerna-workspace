...
export class CometChatButton extends LitElement {
  ...
  @property({ attribute: false }) onClick: any = () => {
  };
  ...
  render() {
    ...
    return html`<button class="cc__button" title=${this.hoverText} @click=${() => { this.onClick ? this.onClick() : null }} style= ${styleMap(this.buttonStyle())}  ?disabled=${this.disabled} >${html`<cometchat-icon  .URL=${this.iconURL} .style=${this.iconStyle}></cometchat-icon>`} ${this.text}</button>`;
  }
  buttonStyle() {
    return {
